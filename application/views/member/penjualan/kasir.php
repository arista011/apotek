	...
	<div class="modal fade bd-example-modal-lg" id="modal-payment" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" style="width:60%">
			<div class="modal-content">
				...
				<section class="panel panel-primary">
					<header class="panel-heading">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Batal</span></button>
			...
			var table = $('#jualdata').DataTable({
				...
				"columns": [{
						...
						"render": function(data, type, row, meta) {
							return '<button id="edit" class="btn btn-info btn-sm" onclick="edit(this)" data-id="' + row.id + '">Ubah</button>';
						}
					},
					{
						"data": "tanggal_jam"
			...
			$.ajax({
				...
				success: function(response) {
					if (response.datarows !== null && response.datarows.length > 0) {
						var data = response.datarows[0];
						$('#id').val(data.id);
						$('#tanggal_jam').val(data.tanggal_jam);
