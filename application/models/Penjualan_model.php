...
class Penjualan_model extends CI_Model
{
    ...
    function submitpayment()
    {
        ...
        foreach ($detailkeranjang->result_array() as $r) {
            // Fetch product name and format price
            ...
            // Prepare data for penjualan_detail insertion
            ...
            if ($r['jenis'] != 'racikan') {
                // Handle stock deduction for non-compounded items
                $kuantiti = $r['kuantiti'];
                while ($kuantiti > 0) {
                    $stok = $this->db->order_by('tgl_expired', 'ASC')
                        ->get_where('kartu_stok', array('kode_item' => $r['kode_item'], 'jumlah_masuk >' => 0), 1);
                    if ($stok->num_rows() == 0) {
                        break; // No more stock available
                    }
                    $stok_row = $stok->row();
                    $stok_terpakai = min($stok_row->jumlah_masuk, $kuantiti);
                    $kuantiti -= $stok_terpakai;
                    ...
                    $stok_input = array(
                        ...
                        'jumlah_masuk' => $stok_row->jumlah_masuk - $stok_terpakai,
                        'jumlah_keluar' => $stok_terpakai,
                        'satuan_kecil' => $r['satuan'],
                        'tgl_expired' => $stok_row->tgl_expired,
                    );
                    $this->db->insert("kartu_stok", $stok_input);
                    $this->db->set('jumlah_masuk', 'jumlah_masuk - ' . (int)$stok_terpakai, FALSE)
                        ->where('id', $stok_row->id)
                        ->update('kartu_stok');
                    $this->db->set('stok', 'stok - ' . (int)$stok_terpakai, FALSE)
                        ->where('kode_item', $r['kode_item'])
                        ->update('master_item');
                }
            } else {
                // Handle stock deduction for compounded items (racikan)
                $racikan = $this->db->get_where('master_racikan', array('kode_racikan' => $r['kode_item']));
                foreach ($racikan->result_array() as $racik) {
                    $kuantiti = $r['kuantiti'] * $racik['jumlah_obat_dipakai'];
                    while ($kuantiti > 0) {
                        $stok = $this->db->order_by('tgl_expired', 'ASC')
                            ->get_where('kartu_stok', array('kode_item' => $racik['kode_obat'], 'jumlah_masuk >' => 0), 1);
                        if ($stok->num_rows() == 0) {
                            break; // No more stock available
                        }
                        $stok_row = $stok->row();
                        ...
                        $this->db->set('jumlah_masuk', 'jumlah_masuk - ' . (int)$stok_terpakai, FALSE)
                            ...
                            ->update('kartu_stok');
                        $this->db->set('stok', 'stok - ' . (int)$stok_terpakai, FALSE)
                            ->where('kode_item', $racik['kode_obat'])
                            ->update('master_item');
                    }
                }
            }
    ...
    {
        $this->db->select('penjualan.*, penjualan_detail.*, penjualan_pembayaran.*, master_admin.nama_admin, master_item.nama_item');
        $this->db->from('penjualan');
        $this->db->join('penjualan_detail', 'penjualan.id = penjualan_detail.id_penjualan');
        $this->db->join('penjualan_pembayaran', 'penjualan.id = penjualan_pembayaran.id_penjualan');
    ...
    public function list_penjualan($limit, $start, $id = null)
    {
        if ($id !== null) {
            $this->db->where('id', $id);
        }
    ...
    {
        // Update data pada tabel penjualan_detail
        for ($i = 0; $i < count($data['id_penjualan']); $i++) {
            $id_penjualan = $data['id_penjualan'][$i];
            $kode_item = $data['kode_item'][$i];
            $harga = $data['harga'][$i];
