...
class Penjualan extends CI_Controller
{
    ...
    public function databank()
    {
        ...
        foreach ($list as $r) {
            ...
            $tomboledit = level_user('penjualan', 'jenispembayaran', $this->session->userdata('kategori'), 'edit') > 0 ? '<li><a href="#" onclick="edit(this)" data-id="' . $this->security->xss_clean($r->singkatan) . '">Ubah</a></li>' : '';
    ...
    public function submitpayment()
    {
        ...
        if (bilanganbulat($post['post_totalbelanja']) > 0 && $total_pembayaran >= bilanganbulat($post['post_totalbelanja'])) {
            $keranjangdetail_input = $simpan->submitpayment(); // Get the return value
            $data['success'] = true;
            $data['message'] = "Transaksi berhasil";
            $data['keranjangdetail_input'] = $keranjangdetail_input; // Include the kode_penjualan in the response
        } else {
    ...
    public function generate_invoice($kode_penjualan)
    {
        $this->load->model('Tools_model');
        $apotekInfo = $this->Tools_model->getApotek();
        $data['nama_apotek'] = $apotekInfo->nama_apotek;
        $data['alamat'] = $apotekInfo->alamat;
