...
class Dashboard extends CI_Controller
{
    ...
    public function produk_kadaluarsa()
    {
        ...
        foreach ($subitem as $r) {
            $expiredDate = new DateTime($r->tgl_expired);
            $today = new DateTime();
            $diffDays = $expiredDate->diff($today)->days;
            $isPast = $expiredDate < $today;

            // Tentukan warna berdasarkan perhitungan
            $bgColor = $isPast ? 'red' : ($diffDays < 30 ? 'yellow' : 'white');
            $fnColor = $isPast ? 'white' : 'black';

            $subArray = [
                'kode_item' => $r->kode_item,
                'nama_item' => $r->nama_item,
                'tgl_expired' => tgl_indo($r->tgl_expired),
        ...
        echo json_encode(['datasub' => $arraysub]);
    }
